/**
 * MotionAnimations - Premium Animation Controller
...
function initHeroAnimations() {
  ...
  if (floatingBadge) {
    ...
    safeAnimate(
      ...
      {
        opacity: [0, 1],
        transform: ['translateX(-50%) scale(0.8)', 'translateX(-50%) scale(1)'],
      },
  ...
  revealLeftElements.forEach((el) => {
    ...
    inView(
      ...
      () => {
        safeAnimate(
          el,
          { opacity: [0, 1], transform: ['translateX(-50px)', 'translateX(0px)'] },
          { duration: config.duration.normal, easing: config.easing }
        );
      },
      { amount: 0.2 }
...
function initMagneticHover() {
  ...
  const buttons = document.querySelectorAll(
    '.btn-primary, .btn-secondary, [data-motion="button"]'
  );
  buttons.forEach((btn) => {
...
function initWhatsAppFAB() {
  const fab = document.querySelector(
    '[aria-label="Chat via WhatsApp"], .whatsapp-fab, a[href*="wa.me"]'
  );
  if (!fab) return;
...
function initImageReveals() {
  ...
  images.forEach((img) => {
    ...
    element.addEventListener('load', () => {
      safeAnimate(element, { opacity: [0, 1] }, { duration: 0.5, easing: config.easing });
    });
  });
}
...
function initAllAnimations() {
  if (prefersReducedMotion) {
    ...
    document
      .querySelectorAll(
        '.hero-title, .hero-subtitle, .hero-badge, .hero-social-proof, .hero-cta, .hero-image, .floating-badge'
      )
      .forEach((el) => {
        (el as HTMLElement).style.opacity = '1';
        (el as HTMLElement).style.transform = 'none';
  ...
  console.log('Motion: All animations initialized ✨');
}
